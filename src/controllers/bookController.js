...
const bookList = async function (req, res) {
  let listbook = await BookModel.find().select({ bookName: 1, authorName: 1 ,_id:0});
...
const getBooksInYear = async function (req, res) {
  ...
  let yearPublished = await BookModel.find({ year: year });
...
const getParticularBooks = async function (req, res) {
  ...
  let particularData = await BookModel.find(data);
  res.send({ msg: particularData });
};

const getXINRBooks = async function (req, res) {
  let INRPrice = await BookModel.find({
...
const getRandomBooks = async function (req, res) {
  let booksReturn = await BookModel.find({
