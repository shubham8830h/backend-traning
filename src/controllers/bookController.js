...
const createBook = async function (req, res) {
  try {
    ...
    if (dataBody.length == 0) {
      return res
        .status(400)
        .send({ status: false, message: "DataBody can not be empty" });
    }
    ...
    if (titleCheck) {
      return res
        .status(400)
        .send({ status: false, message: "This title already exists" });
    }
    ...
    if (ISBN.length < 13 || ISBN.length > 13) {
      return res
        .status(400)
        .send({ status: false, message: "Length of ISBN number is incorrect" });
    }
    ...
    if (isbnCheck) {
      return res
        ...
        .send({
          status: false,
          message: "This ISBN Number is already registerd",
        });
    }
    ...
    if (!subcategory) {
      return res
        .status(400)
        .send({ status: false, mssg: "Subcategory is mandatory" });
    }
    ...
    if (reviews) {
      if (typeof reviews != Number) {
        return res
          .status(400)
          .send({ status: false, message: "Give reviews only in a Number." });
      }
    }
    ...
    if (isDeleted) {
      if (typeof isDeleted != "boolean") {
        return res
          ...
          .send({
            status: false,
            message: "isDeleted can be true or false only",
          });
      }
    }
    ...
    if (!releasedAt) {
      return res
        .status(400)
        .send({ status: false, msg: "Please Provide releasedAt" });
    }
...
const getBookByParam = async function (req, res) {
  try {
    ...
    return res
      ...
      .send({
        status: true,
        message: "Book Lists with Reviews",
        data: { object },
      });
  } catch (err) {
    return res.status(500).send({ status: false, message: err.message });
...
const updateBooks = async function (req, res) {
  try {
    ...
    if (checkIsbn) {
      return res
        .status(400)
        .send({ status: false, message: "This ISBN already exists" });
    }
...
const deletedByParams = async function (req, res) {
  try {
    ...
    let findData = await bookModel
      ...
      .select({ isDeleted: 1, deletedAt: 1 });
    res
      .status(200)
      .send({
        status: true,
        message: "Book deletion is successful",
        data: findData,
      });
  } catch (error) {
